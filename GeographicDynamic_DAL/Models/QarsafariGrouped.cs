using System;
using System.Collections.Generic;

namespace GeographicDynamic_DAL.Models;

public partial class QarsafariGrouped
{
    public double? UniqId { get; set; }

    public double? LiterId { get; set; }

    public string? PhotoN { get; set; }

    public string? Region { get; set; }

    public string? Municipality { get; set; }

    public string? AdmMun { get; set; }

    public string? CityTownVillage { get; set; }

    public double? LandAreaSqM { get; set; }

    public double? LandAreaHa { get; set; }

    public double? Shrubbery { get; set; }

    public double? WoodyPlantPercent { get; set; }

    public double? WoodyPlantQuantity { get; set; }

    public double? VarjisFarti { get; set; }

    public string? WoodyPlantSpecies { get; set; }

    public double? InGoodCondition { get; set; }

    public double? ChoppedDown { get; set; }

    public double? Rampike { get; set; }

    public double? SpeciesMediumAge { get; set; }

    public string? Note { get; set; }

    public string? Company { get; set; }

    public string? FieldOperator { get; set; }

    public string? Date { get; set; }

    public string? GisOperator { get; set; }

    public string? DaTe1 { get; set; }

    public string? OverlapCadCode { get; set; }

    public string? Owner { get; set; }

    public string? LegalPerson { get; set; }

    public string? Owners { get; set; }

    public string? LandFieldOperator { get; set; }

    public string? Note1 { get; set; }

    public string? Date2 { get; set; }

    public string? LandGisOperator { get; set; }

    public string? Note11 { get; set; }

    public string? Date3 { get; set; }

    public string? CadCod { get; set; }

    public double? UniqIdOld { get; set; }

    public double? UniqIdNew { get; set; }

    public string? Uid { get; set; }

    public string? Sakutreba { get; set; }

    public int Id { get; set; }

    public DateOnly? Aleks { get; set; }

    public string? Test1 { get; set; }

    public string? Test2 { get; set; }

    public double? Test3 { get; set; }
}
